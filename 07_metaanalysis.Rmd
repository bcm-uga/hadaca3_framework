...
```{r table_top5_by_dataset, echo=FALSE}
...
if (length(which(results_li_top5$GlobalScore == max(results_li_top5$GlobalScore))) > 5) {
  results_li_top5 = results_li_top5[which(results_li_top5$GlobalScore == max(results_li_top5$GlobalScore)), ]
} else {
  results_li_top5 = results_li_top5[1:5,]
}
...
```{r scatter_plot_top5_by_dataset, warning=FALSE, echo=FALSE, eval=FALSE}
...
df_scatter = do.call(
  what = rbind,
  args = lapply(seq_along(unique(results_li$dataset)), function(data) { # we remove in vivo
    
    prediction_file = sapply(1:nrow(results_li_top5), function(i){
      paste0("pred-li-",paste(c(data, "ref", results_li_top5[i,1:13]),collapse = "_") ,".h5")
    })
    pred = lapply(prediction_file, \(path){read_hdf5(path)})
    
    for (method_i in 1:length(pred)) {
      prediction_data = t(pred[[method_i]]$pred)
...
plot_aggregated_score <- function(data, x_var, y_var, title, facet_var = NULL, add_tooltip_text = FALSE) {
  ...
  # noBOCK is joint with onlyOMIC late integration
  if (y_var != "early_integration" || y_var != "late_integration") {
