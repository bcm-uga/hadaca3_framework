...
results_list <- future_lapply(score_files, process_file)
...
if (list_wd[length(list_wd)] == 'hadaca3_framework') {
  ...
  score_files <- list.files(pattern = 'score-ei*', full.names = TRUE)
  # score_files <- system("find . -maxdepth 1 -type f -name 'score-ei*'", intern = TRUE)
...
if(length(results_list) != 0  ){
  results_ei <- do.call(rbind, Filter(Negate(is.null), results_list))
...
```{r table_top5best_by_dataset, echo=FALSE, eval=FALSE}
