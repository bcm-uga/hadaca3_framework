...
d_cmd = {"nextflow_stub":nextflow_cmd_stub,"nextflow":nextflow_cmd, "snakemake":smk_cmd,"snakemake_dry":smk_cmd_dry }
...
for w_name,work_flow in    d_cmd.items():
    for i in setup_nb :
        ...
        print(cmd.split(' '))
        command = [
        "conda", "run", "-n", "hadaca3framework_env", 
        "nextflow", "run", "00_run_pipeline.nf", "-stub", "-with-report",
        "--setup_folder", "benchmark/setup1/"
        ]
        print(command)
        start_time, end_time, memory_usage, stdout, stderr = run_process(cmd.split(' '))
        # start_time, end_time, memory_usage, stdout, stderr = run_process(command)
        
        process_name = w_name+str(i)
        file_path_stdout = bench_path + "stdout"+process_name+".txt"
        with open(file_path_stdout, "w") as f:
            f.write(stdout)

        file_path_err = bench_path+"stderr"+process_name+".txt"
        with open(file_path_err, "w") as f:
            f.write(stderr)
