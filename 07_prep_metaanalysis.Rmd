...
number_part <- gsub("[^0-9]", "", test_dataset) #problem with mixs with several number number in them. 

number_part = str_sub(str,-1)
name_aprt = str_sub(str,end=str_length(str)-1)
...
name_part = str_sub(str,end=str_length(test_dataset)-1)
...
df_scatter = do.call(
  ...
  args = lapply(levels(results_li$dataset), function(data) { # we remove in vivo
    ...
    name_part = str_sub(str,end=str_length(data)-1)
    ground_truth_name_file = paste0("groundtruth",number_part,'_',name_part,"_pdac.h5")
    ground_truth = read_hdf5(paste0(ground_truth_name_file))$groundtruth
...
```{r scatter_plot_top5worst_by_dataset, warning=FALSE, echo=FALSE, message=TRUE,eval=TRUE}
...
```{r plot_FUN, echo=FALSE, message=TRUE,eval=TRUE}
...
```{r paper_figure_FUN, echo=FALSE, message=TRUE,eval=TRUE}
...
```{r paper_figure_common_var_score_aggreg, echo=FALSE,eval=TRUE}
metric = "score_aggreg"
...
```{r common_var_rmse, echo=FALSE,eval=TRUE}
metric = "rmse"
...
```{r common_var_spearman_row, echo=FALSE,eval=TRUE}
metric = "spearman_row"
